...
while diff>epsilon && i<max_iterations
    ...
    Uentrunc{1} = Ue{1}(:,1:ralgo(idx_s));
    Uentrunc{2} = Ue{2}(:,1:ralgo(idx_s));
    Uentrunc{3} = Ue{3}(:,1:ralgo(idx_s));
    Sentrunc = Se(1:ralgo(idx_s), 1:ralgo(idx_s), 1:ralgo(idx_s));
...
save('val_rauhut_01.mat','r','mu','max_iterations',...

...
% Rauhut 
ralgo = 3; %chosen in validation experiments
...
while diff>epsilon && i<max_iterations
    ...
    %Projection step
    [Ue,Se,sve] = mlsvd(reshape(wr,[n1,n2,n3]));
    Uentrunc{1} = Ue{1}(:,1:ralgo);
    Uentrunc{2} = Ue{2}(:,1:ralgo);
    Uentrunc{3} = Ue{3}(:,1:ralgo);
    Sentrunc = Se(1:ralgo, 1:ralgo, 1:ralgo);
    What = lmlragen(Uentrunc,Sentrunc);
    wr=reshape(What,[n1*n2*n3,1]);
...
save('test_rauhut_01.mat','r','mu','max_iterations',...

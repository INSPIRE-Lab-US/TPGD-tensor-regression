...
% GLRSP   %chosen in validation experiments
s1algo = 22;
s2algo = 22;
s3algo = 18;
...
while diff>epsilon && i<max_iterations
    ...
    %Projection step
    [U1rec,U2rec,U3rec,Drec,What] = sparse_hosvd_v2(tensor(reshape(wr,[n1,n2,n3])),s1algo,s2algo,s3algo,r1,r2,r3);
...
save('test_glrsp_01.mat','r','mu','max_iterations',...
'epsilon','m_vector','a','tensor_iht_arr','noise_sigma',...
's1algo','s2algo','s3algo','no_of_experiments','seed_no')
